...
class CountryService extends FetchService
{
    ...
    /**
     * Fetch a country by code
     * 
     * @param string $code The country code to search for
     * @param array $queryParams Additional query parameters
     * @return Collection
     */
    public function fetchByCode(string $code, array $queryParams = []): Collection
    {
        $codeEndpoint = self::API_URL . '/alpha/' . $code;
        try {
            $response = Http::get($codeEndpoint, $queryParams);
            if ($response->successful()) {
                return collect($response->json())->map(function ($country) {
                    return [
                        'name' => $country['name']['common'] ?? 'Unknown',
            ...
            logger()->error('Error fetching country by code: ' . $e->getMessage());
            return collect([]);
        }
        return collect([]);
    }
